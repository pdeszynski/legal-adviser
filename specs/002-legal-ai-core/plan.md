...
- **Frontend**: Next.js, Refine.dev, Shadcn UI, Tailwind CSS, `react-i18next`, `@graphql-codegen/cli`, GraphQL client (fetch-based or Apollo)
- **Backend**: Nest.js, `@ptc-org/nestjs-query-graphql` + `@ptc-org/nestjs-query-typeorm` (Code-First GraphQL with auto-generated CRUD), `@nestjs/event-emitter`, `@nestjs/bull`, `bull`
  - **IMPORTANT**: Use maintained `@ptc-org/nestjs-query-*` packages, NOT deprecated `@nestjs-query/*` ([docs](https://tripss.github.io/nestjs-query/docs/introduction/getting-started))
...
- **GraphQL-only for frontend-backend communication**:
  - **Standard CRUD**: Use `@ptc-org/nestjs-query-graphql` auto-generated resolvers (filtering, sorting, paging built-in)
  - **Custom Business Logic**: Use raw `@nestjs/graphql` only for non-CRUD mutations (e.g., `generateDocument`, `startAIProcess`)
- REST only for service-to-service (Backend ↔ AI Engine via OpenAPI)
- English-first codebase
- Strong Typing (No `any`)
- Autogenerated Clients (GraphQL codegen for frontend, OpenAPI for backend-to-AI)
