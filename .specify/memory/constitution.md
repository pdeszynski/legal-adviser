...
- **Core Backend**: Nest.js + `@ptc-org/nestjs-query-*` ([docs](https://tripss.github.io/nestjs-query/docs/introduction/getting-started)). GraphQL is the **ONLY** API for frontend-backend communication (including authentication). Database: PostgreSQL. User Management: Nest.js / WorkOS (AuthKit).
  - **Standard CRUD**: Use nestjs-query auto-generated resolvers (filtering, sorting, paging built-in)
  - **Custom Business Logic**: Use raw `@nestjs/graphql` ONLY for non-CRUD operations (e.g., `generateDocument`, AI triggers)
...
- **Autogenerated Clients**:
  - **Internal (Service-to-Service)**: Communication (e.g., NestJS ↔ Python AI Engine) MUST use **REST** with clients **autogenerated** from OpenAPI specs.
  - **Public (Frontend ↔ Backend)**: Communication MUST use **GraphQL** exclusively (queries, mutations, subscriptions). Frontend clients SHOULD be **autogenerated** from backend GraphQL schemas (GraphQL Code Generator or similar). REST endpoints for frontend are FORBIDDEN.
...
**Version**: 1.5.0 | **Ratified**: 2025-12-09 | **Last Amended**: 2026-01-09
