...
export function BillingPaymentMethods({ paymentMethods, onError }: BillingPaymentMethodsProps) {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold">{translate('billing.paymentMethods.title')}</h2>
        <button
      ...
      {!paymentMethods || paymentMethods.length === 0 ? (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-sm">{translate('billing.paymentMethods.addMethodPrompt')}</p>
        </div>
      ) : (
          ...
          {paymentMethods.map((method) => (
            <div key={method.id} className="border border-gray-200 rounded-lg p-4 relative">
              ...
              <p className="text-xs text-gray-500">
                {translate('billing.paymentMethods.expires')} {method.expiryMonth}/
                {method.expiryYear}
              </p>
            </div>
          ))}
